...
function HistoryDayPanel({ day, date, settings, progress, onClose }: { day: CourseDay; date: string; settings: AppSettings; progress: AppProgress; onClose: () => void }) {
  return <div className="v4-history-scrim" role="dialog" aria-modal="true" aria-label={`Day ${day.index} 歷史學習紀錄`}>
    <section className="v4-history-panel">
      ...
      <header><div><span>READ-ONLY HISTORY</span><h2>Day {day.index} · {day.title}</h2><p>{formatTaipeiCourseDate(date)} · 過去日期永久鎖定，不可重新挑戰。</p></div><History/></header>
      <div className="v4-history-lessons">{day.blocks.map((block,index)=><article key={block.id}><span>LESSON {index+1} · {subjectLabel(block.subject)}</span><h3>{block.title}</h3><p>{block.video.title}</p><small>{block.duration} min · {block.missions.length} missions</small></article>)}</div>
      ...
      <footer><ShieldCheck/> 歷史頁只讀取既有紀錄，不提供任何任務完成或獎勵按鈕。</footer>
    </section>
  </div>;
}

export function V4SemesterPage({ settings, progress, trustedDate, cloudStatus, courseDateKey, accessForDay, isDayDone, onOpenLesson, onNavigate, onParentArea }:{
  settings:AppSettings; progress:AppProgress; trustedDate:TrustedTaipeiDate; cloudStatus:string; courseDateKey:(day:CourseDay)=>string; accessForDay:(day:CourseDay)=>CourseDayAccess; isDayDone:(day:CourseDay)=>boolean; onOpenLesson:(day:CourseDay,index:0|1)=>void; onNavigate:(view:ViewKey)=>void; onParentArea:()=>void;
}) {
  ...
  return <V4PageShell settings={settings} progress={progress} active="semester" trustedDate={trustedDate} cloudStatus={cloudStatus} onNavigate={onNavigate} onParentArea={onParentArea}>
    ...
    {historyDay&&<HistoryDayPanel day={historyDay} date={courseDateKey(historyDay)} settings={settings} progress={progress} onClose={()=>setHistoryDay(null)}/>}
  </V4PageShell>;
}

export function V4AchievementsPage({ settings, progress, trustedDate, cloudStatus, onNavigate, onParentArea }:{settings:AppSettings;progress:AppProgress;trustedDate:TrustedTaipeiDate;cloudStatus:string;onNavigate:(view:ViewKey)=>void;onParentArea:()=>void}) {
  return <V4PageShell settings={settings} progress={progress} active="achievements" trustedDate={trustedDate} cloudStatus={cloudStatus} onNavigate={onNavigate} onParentArea={onParentArea}>
...
export function V4ShopPage({ settings, progress, trustedDate, cloudStatus, onNavigate, onParentArea, onUnlock, onToggle }:{settings:AppSettings;progress:AppProgress;trustedDate:TrustedTaipeiDate;cloudStatus:string;onNavigate:(view:ViewKey)=>void;onParentArea:()=>void;onUnlock:(childId:string,itemId:string)=>void;onToggle:(childId:string,itemId:string)=>void}) {
  return <V4PageShell settings={settings} progress={progress} active="shop" trustedDate={trustedDate} cloudStatus={cloudStatus} onNavigate={onNavigate} onParentArea={onParentArea}>
...
export function V4ReportPage({ settings, progress, trustedDate, cloudStatus, isChildDayDone, onNavigate, onParentArea, onSettings }:{settings:AppSettings;progress:AppProgress;trustedDate:TrustedTaipeiDate;cloudStatus:string;isChildDayDone:(childId:string,day:CourseDay)=>boolean;onNavigate:(view:ViewKey)=>void;onParentArea:()=>void;onSettings:()=>void}) {
  return <V4PageShell settings={settings} progress={progress} active="report" trustedDate={trustedDate} cloudStatus={cloudStatus} onNavigate={onNavigate} onParentArea={onParentArea}>
