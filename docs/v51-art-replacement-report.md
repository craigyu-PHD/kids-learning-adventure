...
## V5.2 歷史紀錄（已由 V5.3 規則取代）

> 此段僅保留版本演進，不是現行產品規格。後續實作只能依 [`CURRENT-SPEC.md`](./CURRENT-SPEC.md) 的 V5.3 規則判定權限。

- V5.2 曾讓 90 個學習日的兩堂內容由學期日曆直接開啟，未來日標示「課前預覽」、已過日期標示「複習內容」。
- V5.2 的預覽與複習為唯讀，並曾允許九關內容逐關查看。V5.3 已改為孩子未來只看 teaser、過去只看結果；完整九關僅限家長 PIN。
